...
(function () {
  ...
  /** Encode each path segment for local files (spaces, unicode). */
  ...
  function setCursorLocationMode(enabled, { toast = true } = {}) {
    ...
    const cs = el("settings-cursor");
    if (cs) cs.checked = state.cursorMode;
  ...
  function openCheckpointModal(cpId) {
    ...
    const videoSrc = cp.id === "india_gate_qr" ? "India gate.mp4" : null;
    if (videoSrc) {
  ...
  function bindSettingsForm() {
    ...
    if (c)
      c.addEventListener("change", () => {
        setCursorLocationMode(c.checked, { toast: false });
      });
    if (s)
